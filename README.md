...
## 微服务
  现有的微前端框架有single-spa、qiankun、mooa，single-spa已经实现了大部分框架（vue、react、angular）的启动和卸载处理，但不适用于生产环境，qiankun是基于spa-single实现的以运行在生产环境为目标的微前端服务框架，Mooa是一个仅仅基于angular框架的微前端框架